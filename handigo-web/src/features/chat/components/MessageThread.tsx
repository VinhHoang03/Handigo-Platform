          ...
          return (
            ...
            >
              <div className="group relative max-w-[82%]">
                <div
                  className={`relative rounded-2xl px-4 py-2.5 shadow-sm ${isMine ? "rounded-br-md bg-primary text-on-primary" : "rounded-bl-md border border-outline-variant/30 bg-white text-on-surface"}`}
                >
                  {isEditing ? (
                    <div className="w-[min(260px,65vw)] space-y-2">
                      <textarea
                        ...
                        rows={3}
                        autoFocus
                        className="w-full resize-none rounded-xl border border-outline-variant bg-white p-2 text-sm text-on-surface outline-none focus:border-primary"
                      />
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          disabled={busy}
                          className="rounded-lg bg-white px-3 py-1.5 text-xs font-bold text-primary disabled:opacity-50"
                        >
                          Lưu
                        </button>
                      </div>
                    ...
                    <>
                      ...
                      {message.imageUrl && (
                        <button
                          ...
                          onClick={() =>
                            setPreviewImageUrl(
                              normalizeImageUrl(message.imageUrl),
                            )
                          }
                ...
                {isMine && !isEditing && (
                  <div className="absolute right-full top-1/2 z-10 mr-2 -translate-y-1/2">
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center rounded-full bg-white text-on-surface-variant opacity-100 shadow-md transition sm:opacity-0 sm:group-hover:opacity-100 sm:focus:opacity-100"
                    >
