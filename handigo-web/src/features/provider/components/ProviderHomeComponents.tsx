    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-md">
      <div>
        <div className="flex items-center gap-2 mb-xs">
          {job.status === 'Đang hoạt động' && (
            <span className="bg-primary text-on-primary text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">Đang hoạt động</span>
          )}
      ...
      <div className="flex items-center gap-2">
        {job.status === 'Đang hoạt động' || job.status === 'Active' ? (
          ...
          <span className="px-3 py-1 rounded-full bg-secondary-container text-on-secondary-container text-xs font-bold uppercase">Đã xác nhận</span>
        )}
      </div>
    </div>
  </div>
...
export const WalletWidget: React.FC<WalletWidgetProps> = ({ balance, weeklyEarnings }) => (
  <div className="border border-outline-variant bg-surface-container-lowest p-md rounded-xl">
    ...
    <div className="mb-lg">
      ...
      <p className="text-[40px] font-bold text-on-background leading-tight tabular-nums">{balance}</p>
    </div>
    <div className="space-y-md">
      <div className="flex items-center justify-between p-3 bg-surface-container rounded-xl">
        ...
        <p className="font-semibold text-on-background tabular-nums">{weeklyEarnings}</p>
      </div>
      <button className="w-full py-4 bg-primary text-on-primary rounded-xl font-bold hover:scale-[1.02] transition-transform">Rút tiền ngay</button>
    </div>
  </div>
);
...
export const EarningsChart: React.FC = () => (
  <div className="border border-outline-variant bg-surface-container-lowest p-md rounded-xl overflow-hidden relative min-h-[220px] flex flex-col">
