...
export function RepairQuotationForm({ onSubmit, busy }: RepairQuotationFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-card h-full space-y-md rounded-3xl p-md">
      <div>
        <h3 className="font-headline-md text-on-surface">Tạo báo giá sửa chữa</h3>
        <p className="mt-1 text-sm text-on-surface-variant">
          Ghi nhận kết quả khảo sát và gửi báo giá cho khách hàng xác nhận.
        </p>
      </div>
      ...
      <div className="space-y-sm">
        ...
        {items.map((item, index) => (
          <div
            ...
            className="grid gap-sm rounded-2xl border border-outline-variant/30 bg-white p-sm md:grid-cols-12"
          >
            <input
              value={item.title}
              onChange={(event) => updateItem(index, { title: event.target.value })}
              placeholder="Tên hạng mục"
              className="rounded-xl border border-outline-variant px-3 py-2 md:col-span-4"
            />
            <select
              value={item.itemType}
              onChange={(event) =>
                updateItem(index, { itemType: event.target.value as QuotationItem['itemType'] })
              }
            ...
            <input
              ...
              className="rounded-xl border border-outline-variant px-3 py-2 md:col-span-1"
            />
            <input
              type="number"
              min={0}
      ...
      <div className="grid gap-md md:grid-cols-[1fr_auto] md:items-end">
        <label className="space-y-2">
          <span className="text-label-sm text-on-surface-variant">Giảm giá (VND)</span>
          <input
            type="number"
            min={0}
        ...
        <div className="rounded-2xl bg-primary/5 px-md py-sm text-right">
          ...
          <p className="text-headline-md font-bold text-primary">{formatMoney(finalAmount)}</p>
        </div>
      </div>
