      ...
      <div className="grid flex-1 place-items-center px-7 py-10 text-center">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-on-surface-variant">
            {audience === "CUSTOMER"
              ? "Tôi có thể giúp đặt dịch vụ, gửi yêu cầu hỗ trợ, khiếu nại và tra cứu chính sách Handigo."
              : audience === "PROVIDER"
        ...
        <div className="flex justify-start" aria-live="polite">
          <div className="flex items-center gap-1 rounded-2xl rounded-bl-md border border-outline-variant/30 bg-surface-container-lowest px-4 py-3 shadow-sm">
            {[0, 1, 2].map((index) => (
              <span
                key={index}
                className="h-2 w-2 animate-bounce rounded-full bg-primary/70 motion-reduce:animate-none"
