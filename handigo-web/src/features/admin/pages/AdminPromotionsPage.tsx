...
export default function AdminPromotionsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const inactive = items.filter(
      (item) => item.status === "INACTIVE" || !item.isActive,
    ).length;
  ...
  const openEdit = (voucher: Voucher) => {
    ...
    setForm({
      ...
      minOrderAmount:
        voucher.minOrderAmount == null ? "" : String(voucher.minOrderAmount),
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      setEditing(null);
      setNotice(
        modalMode === "edit" ? "Đã cập nhật voucher." : "Đã tạo voucher mới.",
      );
      await load();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  const confirmToggle = async () => {
    ...
    setNotice("");
    try {
      if (toggleTarget.isActive && toggleTarget.status === "ACTIVE")
        await voucherApi.disable(toggleTarget.id);
  ...
  return (
    <DashboardShell role="ADMIN">
      <div className="space-y-6">
        ...
        <section className="rounded-xl border border-outline-variant/20 bg-surface-container-lowest p-5 shadow-sm">
            ...
            <div className="flex gap-3">
              <select
                ...
                onChange={(event) =>
                  setQuery({
                    ...query,
                    status: event.target.value as VoucherQuery["status"],
                    page: 1,
                  })
                }
          ...
          <AsyncState
            ...
            error={error}
            empty={!items.length}
            emptyMessage="Chưa có voucher phù hợp."
            onRetry={load}
          >
            <VoucherTable
              items={items}
      ...
      <ConfirmDialog
        ...
        title={
          toggleTarget?.isActive && toggleTarget.status === "ACTIVE"
            ? "Tạm dừng voucher"
            : "Kích hoạt voucher"
        }
          ...
          {items.map((voucher) => (
            <tr key={voucher.id} className="hover:bg-surface-container-low">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-on-surface-variant">
                    <span>
                      ...
                      <b>
                        {voucher.minOrderAmount == null
                          ? "-"
                          : money.format(voucher.minOrderAmount)}
                      </b>
                    </span>
                    <span>
                      Sử dụng:{" "}
                    ...
                    <span>
                      Hạn:{" "}
                      <b>
                        {dateTime.format(new Date(voucher.startAt))} -{" "}
                        {dateTime.format(new Date(voucher.endAt))}
                      </b>
                    </span>
                  </div>
                </div>
              ...
              <td className="py-4 align-top">
                <StatusBadge value={statusValue(voucher)} />
              </td>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <FormInput
            ...
            value={form.code}
            onChange={(value) =>
              onChange({ ...form, code: value.toUpperCase() })
            }
          />
          <FormInput
            label="Tên hiển thị"
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <FormInput
            ...
            onChange={(value) =>
              onChange({ ...form, maxDiscountAmount: value })
            }
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            ...
            <select
              ...
              onChange={(event) =>
                onChange({
                  ...form,
                  status: event.target.value as VoucherFormState["status"],
                })
              }
              className="w-full rounded-xl border border-outline-variant bg-surface p-3 outline-none focus:ring-2 focus:ring-primary/30"
            >
