...
export default function AdminUsersPage() {
  ...
  const update = async () => {
    ...
    try {
      setBusy(true);
      await adminApi.updateUserStatus(target._id, target.status === 'active' ? 'locked' : 'active');
      setTarget(null);
      await load();
    } finally {
      setBusy(false);
  ...
  return (
    <DashboardShell role="ADMIN">
      <header><h1 className="text-headline-lg font-bold">Quản lý người dùng</h1><p className="text-on-surface-variant">Tìm kiếm, xem chi tiết và quản lý trạng thái tài khoản.</p></header>
      ...
      <AsyncState loading={loading} error={error} empty={!users.length} emptyMessage="Không có người dùng phù hợp." onRetry={load}>
        <UserTable users={users} onView={setDetail} onToggleStatus={setTarget} />
      </AsyncState>
