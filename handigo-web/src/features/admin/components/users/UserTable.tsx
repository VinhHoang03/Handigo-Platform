        ...
        <tbody>
          {users.map((user) => (
            <tr key={user._id} className="border-t border-outline-variant/30">
              <td className="p-4"><div className="flex items-center gap-3"><InitialsAvatar name={user.fullName} src={user.avatar} className="h-10 w-10" /><div><p className="font-semibold">{user.fullName}</p><p className="text-sm text-on-surface-variant">{user.email}</p></div></div></td>
              <td className="p-4">{roleLabel[user.role]}</td>
              <td className="p-4 tabular-nums">{new Date(user.createdAt).toLocaleDateString('vi-VN')}</td>
